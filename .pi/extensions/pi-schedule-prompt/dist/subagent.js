...
export function resolveModel(registry, modelStr) {
...
export function describeAvailableModels(registry) {
    const available = registry.getAvailable();
    if (available.length === 0)
        return "No models with configured auth.";
...
export async function runSubagentOnce(ctx, prompt, modelStr, signal, options = {}) {
    try {
        ...
        const isEnabled = (v) => v === true || (Array.isArray(v) && v.length > 0);
        const getNameList = (v) => Array.isArray(v) && v.length > 0 ? v : undefined;
        ...
        const loader = new DefaultResourceLoader({
            ...
            ...(extList && {
                extensionsOverride: (base) => ({
                    ...base,
                    extensions: base.extensions.filter((ext) => extList.some((name) => ext.path.toLowerCase().includes(name.toLowerCase()))),
                }),
            }),
            ...(skillList && {
                skillsOverride: (base) => ({
                    ...base,
                    skills: base.skills.filter((skill) => skillList.includes(skill.name || "")),
                }),
            }),
        });
        await loader.reload();
