...
export default async function (pi) {
    ...
    const tool = createCronTool(() => storage, () => scheduler, () => settings.defaultJobScope ?? "session");
    ...
    const autoCleanupDisabledJobs = (ctx) => {
        ...
        handler: async (_args, ctx) => {
            ...
            switch (action) {
                case "Jobs": {
                    ...
                    await ctx.ui.custom((tui, theme, _kb, done) => new JobsView(storage, scheduler, mySessionId, wrappedRunAdd, theme, () => tui.requestRender(), () => done(undefined)), {
                        overlay: true,
                ...
                case "Settings": {
                    ...
                    while (true) {
                        ...
                        if (choice.startsWith("Widget visibility:")) {
                            ...
                            if (!persisted) {
                                ctx.ui.notify(`Widget ${next ? "shown" : "hidden"} (session only; failed to persist)`, "warning");
                            }
                        }
                            ...
                            if (!persisted) {
                                ctx.ui.notify(`Bind new jobs to session: ${next === "session" ? "yes" : "no"} (session only; failed to persist)`, "warning");
                            }
                        }
                    }
                }
