...
export class CronScheduler {
    ...
    start() {
        ...
        for (const job of this.storage.getAllJobs()) {
            if (!CronScheduler.isLoadedFor(job, mySessionId))
                continue;
    ...
    static isLoadedFor(job, sessionId) {
    ...
    async executeJob(job) {
        ...
        if (!fresh?.enabled)
            return;
        if (!CronScheduler.isLoadedFor(fresh, this.ctx.sessionManager.getSessionId()))
            return;
     ...
     */
    static parseRelativeTime(delta) {
     ...
     */
    static validateSchedule(type, schedule) {
        if (type === "interval") {
        ...
        if (type === "once") {
            ...
            if (relative)
                return { ok: true, schedule: relative };
     ...
     */
    static describeSchedule(type, schedule) {
        if (type === "interval")
...
export function humanizeCron(expression) {
    const normalized = expression.trim();
    if (HUMANIZED_CRON[normalized])
        return HUMANIZED_CRON[normalized];
