...
export class CronWidget {
    ...
    ctx;
    unsubscribe;
    constructor(storage, scheduler, pi, isVisible, sessionId = undefined) {
    ...
    renderWidget(width, theme) {
        ...
        container.addChild(new Text(theme.fg("accent", theme.bold("Scheduled Prompts")) + theme.fg("dim", ` (${uniqueJobs.length} jobs)`), 1, 0));
        ...
        for (const job of uniqueJobs) {
            ...
            if (scheduleRaw.length > 15)
                scheduleRaw = `${scheduleRaw.substring(0, 12)}...`;
            ...
            const modelBadge = job.model
                ? " " + theme.fg("accent", `[${job.model.length > 12 ? job.model.substring(0, 9) + "..." : job.model}${job.notify ? "!" : ""}]`)
                : "";
            // Combine into a row with proper spacing
            lines.push(` ${statusIcon} ${nameText} ${scheduleText} ${promptText} ${nextText} ${lastText} ${countText}${modelBadge}`);
        }
