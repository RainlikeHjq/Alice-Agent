...
export async function runAddFlow(ctx, storage, scheduler, settings, mySessionId) {
    ...
    const scopeChoice = await ctx.ui.select("Scope", defaultIsSession ? [SCOPE_SESSION, SCOPE_SHARED] : [SCOPE_SHARED, SCOPE_SESSION]);
    if (!scopeChoice)
        return;
    ...
    const confirmed = await ctx.ui.confirm("Confirm", `Save "${name}"?\nSchedule: ${human}\nScope: ${scopeLabel}\nPrompt: ${prompt}`);
    if (!confirmed)
        return;
    const job = {
        id: nanoid(10),
